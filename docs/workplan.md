...
| T-A2 | Settings + env catalog: every §12 knob as a documented reader (incl. `IG_TIMEOUT_MS`, `IG_LOG_LEVEL`); machine-readable catalog for generators; stderr JSON logger with levels + `safeUrl` | `src/core/settings.ts`, `src/core/log.ts` | G1 | — | M | **PARTIAL — one knob left** (2026-07-30). Readers, logger and the machine-readable catalog are DONE; `IG_WRITE_JOURNAL` now appears in `.env.example`, architecture §12 and the generated README. What is still true: it has **no home in `core/settings.ts`** — `src/mcp/write-mode.ts` reads it straight off `env`, and says so in its own comment. Closing this means adding it to `Settings`/`DEFAULT_SETTINGS`/`loadSettings` and having the gate read the setting instead of the environment. |
...
| T-R7 | `SECURITY.md`, `CHANGELOG.md`, release checklist, three-channel version-drift test | those files | — | S | **DONE** — the owed correction landed: `CHANGELOG.md` now states plainly that `0.0.1` is a pre-release placeholder, not a published version. |
